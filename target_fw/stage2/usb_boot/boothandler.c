...
int NAND_OPS_Handle(u8 *buf)
{
	...
	case NAND_READ_OOB:
		...
		memset(Bulk_in_buf,0,ops_length*CFG_NAND_PS);
		ret_dat = nand_read_oob(Bulk_in_buf,start_addr,ops_length);
		handshake_PKT[0] = (u16) ret_dat;
		handshake_PKT[1] = (u16) (ret_dat>>16);
		HW_SendPKT(1,(u8 *)Bulk_in_buf,ops_length*CFG_NAND_PS);
		udc_state = BULK_IN;		
		break;
	case NAND_READ_RAW:
		...
		case OOB_ECC:
			...
			HW_SendPKT(1,(u8 *)Bulk_in_buf,ops_length*(CFG_NAND_PS + CFG_NAND_OS));
			handshake_PKT[0] = (u16) ret_dat;
			handshake_PKT[1] = (u16) (ret_dat>>16);
			udc_state = BULK_IN;
			break;
	...
	case NAND_PROGRAM:
		...
		ops_length -= 1;
		if(option == NO_OOB)
			check_len = ops_length * CFG_NAND_PS;
		else
			check_len = ops_length * (CFG_NAND_PS + CFG_NAND_OS);
		file_crc32 = (unsigned long)Bulk_out_buf[check_len / 4];
		check_crc32 = do_crc32((unsigned char *)Bulk_out_buf, check_len);
...
void Board_Init()
{
	...
	//Init nand flash
	nand_init_4760(CFG_NAND_BW, CFG_NAND_RC, CFG_NAND_PS,
		   CFG_NAND_PPB, CFG_NAND_BCHBIT, CFG_NAND_ECCPOS,
		   CFG_NAND_BBPOS, CFG_NAND_BBPAGE, CFG_NAND_FORCEERASE, CFG_NAND_OS, CFG_NAND_BCHSTYLE);
